...
const turnLoop = [
  ...
  "Toll tile: settle it the moment the player decides — action='pay_toll' (they pay) or action='serve_toll' (they did the landlord's task instead, no coins charged). Do NOT carry that decision to the next roll: if you forget the toll arg there, it silently defaults to pay and takes their coins even though they served. If coins were already taken by mistake, call serve_toll again with the same who: it refunds that toll. A player who did the errand must never end up out of pocket.",
...
const actionMap = {
  ...
  toll: "Settle it the moment it is decided: game_action action='pay_toll' (they pay) or action='serve_toll' (they did the landlord's task instead, no coins charged). Deferring to the next roll's toll='pay'/'serve' still works, but if you forget that arg the next roll silently defaults to pay and charges them anyway. Already charged by mistake? Call serve_toll again with the same who — it refunds that toll and re-files it as an errand. Never tell a player 'sorry, already deducted'.",
...
const rollDescription = [
  ...
  "For toll, prefer settling on the spot via game_action pay_toll/serve_toll. A forgotten toll arg here defaults to pay and charges a player who actually did the landlord's task — if that happens, game_action serve_toll with the same who refunds it.",
