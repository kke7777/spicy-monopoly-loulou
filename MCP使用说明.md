...
- `render_game`：可选的 MCP Apps 互动界面；读取权威局面，并提供掷骰、任务结算、过路费/对决、手牌、身份能力和 404 暂停控件。
- `game_admin`：少用的管理动作，例如 `delete_game`、`clear_pair_history`、`submit_feedback`。

MCP 菜单保持为 7 个高层工具，避免客户端每轮都把一大串细碎工具 schema 塞进上下文。完整游戏能力仍在 `roll` / `game_action` / `game_info` / `game_admin`；`render_game` 的按钮只是调用这些权威工具并用真实返回刷新界面，不会在浏览器里自造骰点或金币。
...
- 想显示互动界面时，在 `new_game` 后调用一次 `render_game`：传真实 `game_id`，把系统与任务字段原样放进对应参数；AI 自己写的角色剧情只能放在 `story_text`，不能作为引擎事实。之后玩家在界面中点击时，组件会直接调用 `roll` / `game_action` 并自行刷新；组件发出的回执以 `【互动棋盘已执行，请勿重复调用…】` 开头，AI 只能接剧情，绝不能重复调用工具。若 AI 因聊天文字主动调用了引擎，再用 `render_game` 重同步界面。
